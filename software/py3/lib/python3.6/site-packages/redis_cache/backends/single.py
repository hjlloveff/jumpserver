...
class RedisCache(BaseRedisCache):
    ...
    def get_client(self, key, write=False):
        if write and self.main_client is not None:
    ...
    def delete_many(self, keys, version=None):
        ...
        if versioned_keys:
            self._delete_many(self.main_client, versioned_keys)
    ...
    def clear(self, version=None):
        ...
        """
        if version is None:
            self._clear(self.main_client)
        else:
            self.delete_pattern('*', version=version)

    def get_many(self, keys, version=None):
        versioned_keys = self.make_keys(keys, version=version)
        return self._get_many(self.main_client, keys, versioned_keys=versioned_keys)
    ...
    def set_many(self, data, timeout=DEFAULT_TIMEOUT, version=None):
        ...
        if timeout is None:
            ...
            return self._set_many(self.main_client, new_data)

        pipeline = self.main_client.pipeline()
        for key in versioned_keys:
    ...
    def incr_version(self, key, delta=1, version=None):
        ...
        return self._incr_version(self.main_client, old, new, delta, version)
